...
public sealed class PropertyRepository(AppDbContext dbContext) : IPropertyRepository
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            query = query.Where(x => x.PropertyTypeId == filter.PropertyTypeId);
        }
        ...
        {
            query = query.Where(x => x.Price.Amount >= filter.MinPrice);
        }

        if (filter.MaxPrice is not null)
        {
            query = query.Where(x => x.Price.Amount <= filter.MaxPrice);
        }

        if (filter.MinArea is not null)
        {
            query = query.Where(x => x.Area.Value >= filter.MinArea);
        }

        if (filter.MaxArea is not null)
        {
            query = query.Where(x => x.Area.Value <= filter.MaxArea);
        }

        if (filter.Rooms is not null)
        {
            query = query.Where(x => x.Rooms == filter.Rooms);
        }
    ...
    public Task AddAsync(Property property, CancellationToken ct = default) =>
        dbContext.Properties.AddAsync(property, ct).AsTask();
