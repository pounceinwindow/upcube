...
{
    private const string UnavailableMessage = "Local AI is unavailable.";
    ...
    private static LocalLlmResult Unavailable()
    {
        return new LocalLlmResult(string.Empty, false, UnavailableMessage);
    }

    private static string NormalizeContent(string content)
    {
        var text = content.Trim();
        if (text.Length == 0 || EndsWithSentence(text)) return text;

        var lastSentenceEnd = text.LastIndexOfAny(['.', '!', '?', '…']);
        if (lastSentenceEnd >= 24) return text[..(lastSentenceEnd + 1)].Trim();

        var lastSoftBreak = text.LastIndexOfAny([',', ';', ':', '(']);
        if (lastSoftBreak >= 24) text = text[..lastSoftBreak].Trim();
