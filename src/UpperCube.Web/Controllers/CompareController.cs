    ...
    IStringLocalizer<SharedResource> localizer) : Controller
{
    public async Task<IActionResult> Index(CancellationToken ct)
    {
        ...
        {
            return View(new CompareModelView { IsAuthenticated = false });
        }
    ...
    public async Task<IActionResult> Add(int propertyId, string? returnUrl, CancellationToken ct)
    {
        ...
        {
            TempData["CompareInfo"] = localizer["CompareAlreadyAdded"].ToString();
            return RedirectBack(returnUrl);
        }

        if (comparison.Items.Count >= CompareModelView.MaxItems)
        {
            TempData["CompareError"] = localizer["CompareLimitReached"].ToString();
            return RedirectBack(returnUrl);
        }

        var usedPositions = comparison.Items.Select(x => x.Position).ToHashSet();
        var position = Enumerable.Range(1, CompareModelView.MaxItems).First(x => !usedPositions.Contains(x));
        ...
        TempData["CompareSuccess"] = localizer["CompareAdded"].ToString();
        return RedirectBack(returnUrl);
    }
    ...
    public async Task<IActionResult> Remove(int propertyId, string? returnUrl, CancellationToken ct)
    {
        ...
        if (item is not null)
        {
            ...
            await unitOfWork.SaveChangesAsync(ct);
            TempData["CompareSuccess"] = localizer["CompareRemoved"].ToString();
        }
    ...
    public async Task<IActionResult> Clear(CancellationToken ct)
    {
        ...
        if (comparison is not null && comparison.Items.Count > 0)
        {
            ...
            await unitOfWork.SaveChangesAsync(ct);
            TempData["CompareSuccess"] = localizer["CompareCleared"].ToString();
        }
